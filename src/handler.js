...
const updateUser = async (request, h) => {
    ...
    try {
        const query = 'UPDATE table_user SET user_name = ?, user_born = ?, user_gender = ?, user_height = ?, user_weight = ? WHERE user_id = ?';
        
        // will add userId later
        await new Promise((resolve, reject) => {
            connection.query(query, [name, born, gender, height, weight, userId], (err, rows, field) => {
                if (err) {
                    reject(err);
                } else {
                    resolve();
...
// addConsumeCalorie is still unsure
